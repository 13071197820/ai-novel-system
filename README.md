...
1. **安装 VS Code 插件**
   - 安装 [Cline AI Flow](https://marketplace.visualstudio.com/items?itemName=Cline.ai-flow)
   - 安装 [ChatGPT – Official](https://marketplace.visualstudio.com/items?itemName=openai.chatgpt-vscode)
   - 建议安装 Markdown Preview Enhanced（预览 prompt 效果）

2. **项目初始化**
   ```bash
   mkdir ai-novel-system
   cd ai-novel-system
   mkdir prompts output

⚠️ 这些变量将在 Cline 的 workflow.yaml 中被定义为 全局上下文变量，可随时修改。
...
| 📖 12 | 第一章生成 | 08_chapter_writer.prompt.md     | outline, characters, tone     | output/chapter_01.txt    |


🧠 四、Cline 自动化执行方式（核心逻辑）

Cline 支持 YAML 级工作流描述，可实现链式自动执行。
...
每个 .prompt.md 文件可单独运行，验证效果。

当你修改某个模块输出后，可仅重跑下游节点（例如修改力量体系后，只需重跑从 03_character_design 开始的模块）。

输出结果可用 Markdown Preview 检查格式，或导入 JSON viewer。

若使用多模型协同（如 GPT-5 + Claude 3.5），可将不同模块分配至不同模型以提高生成质量。
